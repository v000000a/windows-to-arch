...
int main() {
    ...
    setup << "#!/bin/bash\n"
          << "set -e\n\n"
          << "echo \"=== Arch installation started ===\"\n"
          ...
          << "  echo 'arch-deformator' > /etc/hostname\n"
          ...
          << "CHROOT\n\n"
          << "# Repurpose original Windows partition\n"
          << "WIN=$(blkid -t TYPE=ntfs -o device | head -n1)\n"
          << "if [ -n \"$WIN\" ]; then\n"
          << "  echo \"Repurposing Windows partition $WIN...\"\n"
          << "  mkfs.btrfs -f -L windows_data \"$WIN\"\n"
          << "  btrfs device add \"$WIN\" /mnt\n"
          << "fi\n\n"
